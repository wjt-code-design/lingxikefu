...
与 per-user 每日用量表 ``quotas``（models/quota.py，计数/防超卖）无关——本表是
**全局设置 KV**：key 覆盖 app.core.config 中的运行时配置项（如 ``quota.daily_limit``
→ ``DAILY_QUOTA_LIMIT``），值统一 JSON 列。首个消费者：QuotaService.daily_limit()
的动态上限（大促免重启上调），写入通道为 admin PUT /admin/settings/quota。
