...
#: 边界用 ASCII lookaround 而非 \b（H1 修复）：Unicode 模式下 CJK 属 \w，\b 对
#: 汉字紧贴订单号（"订单SO123到哪了"）不成立 → 漏提取；lookaround 放行汉字邻接，
#: 同时防长 ASCII 标识符中段误配（左邻/右邻为字母数字则不命中）。
_ORDER_RE = re.compile(
    r"(?<![A-Za-z0-9])[A-Z]{2,}-\d{3,}(?:-\d+)?(?![0-9A-Za-z])"
    r"|(?<![A-Za-z0-9])[A-Z]{2,}\d{4,}[0-9A-Z]*(?![0-9A-Za-z])"
)
